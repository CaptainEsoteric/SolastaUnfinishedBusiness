...
public sealed class TempestKnight : AbstractSubclass
{
    private const string Name = "TempestKnight";

    private const ActionDefinitions.Id ForcePoweredStrikeToggle =
        (ActionDefinitions.Id)ExtraActionId.ForcePoweredStrikeToggle;

    internal static readonly FeatureDefinitionPower TempestCharges = FeatureDefinitionPowerBuilder
        .Create($"Power{Name}TempestCharges")
    ...
    public TempestKnight()
    {
        ...
        var additionalDamageLightningStrike = FeatureDefinitionAdditionalDamageBuilder
        ...
        var conditionForcePoweredStrike = ConditionDefinitionBuilder
            ...
            .SetFeatures(additionalDamageLightningStrike)
        ...
        var conditionStormshield = ConditionDefinitionBuilder
            .Create($"Condition{Name}Stormshield")
            .SetGuiPresentation(Category.Condition, ConditionDefinitions.ConditionShielded)
            .SetPossessive()
            .SetFeatures(
                MagicAffinityConditionShielded,
                FeatureDefinitionAttributeModifierBuilder
                    .Create($"AttributeModifier{Name}Stormshield")
                    .SetGuiPresentation($"Condition{Name}Stormshield", Category.Condition, Gui.NoLocalization)
        ...
        var powerStormshield = FeatureDefinitionPowerSharedPoolBuilder
            .Create($"Power{Name}Stormshield")
            .SetGuiPresentation(Category.Feature)
            .SetSharedPool(ActivationTime.NoCost, TempestCharges)
            .SetEffectDescription(
                EffectDescriptionBuilder
                    .Create()
                    .SetDurationData(DurationType.Round, 1, TurnOccurenceType.EndOfTurn)
                    .SetTargetingData(Side.Ally, RangeType.Distance, 6, TargetType.IndividualsUnique)
                    .SetEffectForms(EffectFormBuilder.ConditionForm(conditionStormshield))
                    .SetParticleEffectParameters(FeatureDefinitionPowers.PowerTraditionOpenHandTranquility)
                    .UseQuickAnimations()
                    .Build())
            .AddToDB();

        powerStormshield.AddCustomSubFeatures(
            ModifyPowerVisibility.Hidden,
            new AttackBeforeHitPossibleOnMeOrAllyKineticBarrier(powerStormshield));
        ...
        actionAffinityForcePoweredStrikeToggle.AddCustomSubFeatures(
            new ValidateDefinitionApplication(ValidatorsCharacter.HasAvailablePowerUsage(TempestCharges)),
        ...
        var featureSetPsionicAdept = FeatureDefinitionFeatureSetBuilder
            ...
            .SetGuiPresentation(Category.Feature)
            .AddFeatureSet(TempestCharges, powerPsionicAdeptProne, powerPsionicAdeptPush)
            .AddToDB();
        ...
        var powerPsionicPropulsion = FeatureDefinitionPowerSharedPoolBuilder
            ...
            .SetSharedPool(ActivationTime.BonusAction, TempestCharges)
            .SetEffectDescription(
                EffectDescriptionBuilder
                    .Create()
                    .SetDurationData(DurationType.Round)
        ...
        var powerTelekineticGrasp = FeatureDefinitionPowerSharedPoolBuilder
            ...
            .SetSharedPool(ActivationTime.Action, TempestCharges)
            .SetShowCasting(false)
            .AddCustomSubFeatures(
                new ValidatorsValidatePowerUse(
                    c => c.GetRemainingPowerUses(powerTelekineticGraspOncePerLong) == 0,
        ...
        Subclass = CharacterSubclassDefinitionBuilder
            ...
            .AddFeaturesAtLevel(6, BuildPowerModifier(TempestCharges, 6))
            .AddFeaturesAtLevel(7, featureSetPsionicAdept, featureSetPsionicPropulsion)
            .AddFeaturesAtLevel(9, BuildPowerModifier(TempestCharges, 9))
            .AddFeaturesAtLevel(10, featureForceOfWill)
            .AddFeaturesAtLevel(12, BuildPowerModifier(TempestCharges, 12))
            .AddFeaturesAtLevel(15, BuildPowerModifier(TempestCharges, 15), featureSetForceBulwark)
            .AddFeaturesAtLevel(18, BuildPowerModifier(TempestCharges, 18), featureSetTelekineticGrasp)
            .AddToDB();
    }
    ...
    private static void CreateStormShield()
    {
        var conditionStormShield = ConditionDefinitionBuilder
           .Create($"Condition{Name}StormShield")
           .SetGuiPresentation(Category.Condition, ConditionDefinitions.ConditionShielded)
        ...
        IAttackBeforeHitConfirmedOnEnemy, IPhysicalAttackFinishedByMe
    {
            ...
            bool criticalHit)
        {
            ...
            if (!attacker.OnceInMyTurnIsValid("ForcePoweredStrike") ||
                rulesetAttacker.GetRemainingPowerUses(TempestCharges) == 0 ||
            ...
            rulesetAttacker.UpdateUsageForPower(TempestCharges, 1);
    ...
    private sealed class ModifyAdditionalDamageFormPoweredStrike : IModifyAdditionalDamageForm
    {
            ...
            DamageForm damageForm)
        {
            ...
            var chaMod = GetChaModifier(rulesetAttacker);

            damageForm.BonusDamage = chaMod;
    ...
    private sealed class ActionItemDiceBoxForcePoweredStrike : IActionItemDiceBox
    {
        ...
        {
            return (GetForcePoweredStrikeSize(character), character.GetRemainingPowerUses(TempestCharges),
        ...
        FeatureDefinitionPower powerForceBulwark) : IModifyEffectDescription
    {
        ...
        public EffectDescription GetEffectDescription(
            ...
            RulesetEffect rulesetEffect)
        {
            var intMod = GetChaModifier(character);
